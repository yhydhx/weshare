...
def login(request):
    return render(request, 'dota/login.html')
...
def loginCertifacate(request):
    if request.method == 'POST':
        ...
        if user.password == password:
            ...
            return HttpResponseRedirect('/dota/items/show/')
        else:
...
def items(request, method, Oid):
    ...
    if method == 'addSelect':
        ...
        select.save()
        # Oid = news.id
        return HttpResponseRedirect('/dota/items/select/')
    elif method == 'select':
        username = request.session['adminname']
        selects = Select.objects.filter(username=username)
        select_ids = []
        for select_single in selects:
            select_ids.append(select_single.item_id)
        NewData = []
        Data =  crawl_data()
        for single_data in Data:
            if single_data['item_id'] in select_ids:
                NewData.append(single_data)


        return render(request, 'dota/showItemList.html', {'object': NewData})
    elif method == 'detail':
        item_id = Oid
        details = Item.objects.filter(item_id = item_id)
        item_name = details[0].item_name

        return render(request, 'dota/showItemDetail.html', {'object': details,"item_name":item_name})
        ...
        Data =  crawl_data()
        return render(request, 'dota/showItemList.html', {'object': Data})
    else:
...
def crawl_data():
    ...
    for i in range(36):
        ...
        for ul in ul_list:
            #print li['class']
            if "list-item4" in ul['class']:
                li_list = ul.find_all("li")
                for li in li_list:
                    item_dict = analyze_item(li)

                    if item_dict['item_type'] == "S":
                        add_item(item_dict)
...
def add_item(item_dict):
    item = Item(
            ...
            item_crawl_date = datetime.datetime.now()
        )
    item.save()
    return 1

def analyze_item(soup_object):
    item_dict = {}

    sold_link = soup_object.find_all('a')[0].get("href")
    sold_id_type =  sold_link.split(".")[0].split("/")[-1].split("-")
    sold_id = sold_id_type[0]
    sold_type = sold_id_type[1]
    span_list = soup_object.find_all("span")
    item_name = span_list[0].text.encode("utf8")
    item_price =  span_list[2].text.split(" ")[-1]
    item_number = span_list[3].text.encode("utf8")[:-9]
    
    item_dict['item_type'] = sold_type
    item_dict['item_id'] = sold_id
    item_dict['item_name'] = item_name
    item_dict['item_price'] = item_price
    item_dict['item_amount'] = item_number
    item_dict['item_crawl_date'] = datetime.datetime.now()
    return item_dict
...
def func(request, method, Oid):
    ...
    elif method == "dating_info":
        dating_users = Dating.objects.all()
        output_file = file( settings.UPLOAD_PATH + 'files/' + 'dating_info.csv','w')
        output_file.write("姓名\t性别\t星座\t地域\t大学\t年级\t性格特点\t微信\t邮箱\t介绍\tTa的性别\tTa的星座\tTa的特点\t图片\t提交时间\n")
        for host_atom in dating_users:
            
            name = host_atom.name.encode("utf-8")
            gender = str(host_atom.gender)
            star = host_atom.star.encode("utf-8")
            location = host_atom.location.encode("utf-8")
            univ = host_atom.univ.encode("utf-8")
            grade = host_atom.grade.encode("utf-8")
            wechat = host_atom.wechat.encode("utf-8")
            personal = host_atom.personal.encode("utf-8")
            email = host_atom.email.encode("utf-8")
            intro = host_atom.intro.encode("utf-8")
            ta_gender = str(host_atom.ta_gender)
            ta_star = host_atom.ta_star.encode("utf-8")
            ta_personal = host_atom.ta_personal.encode("utf-8")
            image = host_atom.image
            upload_time = host_atom.upload_time.strftime("%Y-%m-%d %H:%I:%S")
            output_file.write(name)
            output_file.write("\t")
            output_file.write(gender)
            output_file.write("\t")
            output_file.write(star)
            output_file.write("\t")
            output_file.write(location)
            output_file.write("\t")
            output_file.write(univ)
            output_file.write("\t")
            output_file.write(grade)
            output_file.write("\t")
            output_file.write(personal)
            output_file.write("\t")
            output_file.write(wechat)
            output_file.write("\t")
            output_file.write(email)
            output_file.write("\t")
            output_file.write(intro)
            output_file.write("\t")
            output_file.write(ta_gender)
            output_file.write("\t")
            output_file.write(ta_star)
            output_file.write("\t")
            output_file.write(ta_personal)
            output_file.write("\t")
            output_file.write(image)
            output_file.write("\t")
            output_file.write(upload_time)
            output_file.write("\n")

        return HttpResponseRedirect("/files/files/dating_info.csv")
